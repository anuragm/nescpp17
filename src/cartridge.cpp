...
void cartridge::print_debug_info() {
  ...
  for (int ii = 0; ii < 16; ii++) std::printf("0x%x ", header[ii]);
  ...
  std::printf("Mirroring type is %s.\n",
              mirroring_type == cartridge::mirror_type::horiz ? "Horizontal" : "Vertical");
}
...
bool cartridge::check_rom() {
  ...
  for (u8 ii = 8; ii < 16; ii++) is_valid &= (header[ii] == 0);
